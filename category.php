...
if (function_exists('cs_get_option')):
    //category sidebar
    $category_template_sidebar_id = cs_get_option( 'category_template_sidebar_'.$this_cat_id );
    $category_template_sidebar = cs_get_option( 'category_template_sidebar' );
    if(!empty($category_template_sidebar_id)) {
        $top_news_cat_sidebar = cs_get_option( 'category_template_sidebar_'.$this_cat_id );    
    }else if(!empty($category_template_sidebar)) {
    ...
    //category slider
    if(cs_get_option( 'category_slider_'.$this_cat_id ) == 'true') {
        $top_news_cat_slider = cs_get_option( 'category_slider_'.$this_cat_id );
        $top_news_cat_slider_style = cs_get_option( 'default_category_slider_'.$this_cat_id );    
        $top_news_cat_slider_limit = cs_get_option( 'category_slider_post_limit_'.$this_cat_id );    
        $top_news_cat_slider_tag = cs_get_option( 'category_slider_tag_'.$this_cat_id );    
    } else {
    ...
    $category_template_post_style_id = cs_get_option( 'category_template_post_style_'.$this_cat_id );
    $category_column_id = cs_get_option( 'category_column_'.$this_cat_id );
    $category_excerpt_id = cs_get_option( 'category_excerpt_'.$this_cat_id );
    $category_excerpt_limit_id = cs_get_option( 'category_excerpt_limit_'.$this_cat_id );
    $category_readmore_id = cs_get_option( 'category_readmore_'.$this_cat_id );

    if(!empty($category_template_post_style_id)) {
        $top_news_cat_post_style = cs_get_option( 'category_template_post_style_'.$this_cat_id );
    } else {
    ...
    if(!empty($category_column_id)) {
        $top_news_cat_post_column = cs_get_option( 'category_column_'.$this_cat_id );
    } else {
        $top_news_cat_post_column = cs_get_option( 'category_column' );
    }
    if(!empty($category_excerpt_id)) {
        $is_excerpt = cs_get_option( 'category_excerpt_'.$this_cat_id );
    } else {
        $is_excerpt = cs_get_option( 'category_excerpt' );
    }

    if(!empty($category_excerpt_limit_id)) {
        $excerpt_limit = cs_get_option( 'category_excerpt_limit_'.$this_cat_id );
    } else {
        $excerpt_limit = cs_get_option( 'category_excerpt_limit' );
    }

    if(!empty($category_readmore_id)) {
        $is_readmore = cs_get_option( 'category_readmore_'.$this_cat_id );
    } else {
